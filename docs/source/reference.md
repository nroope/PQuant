...
The most important part of the library is a user-defined config yaml file. It has five separate sections: training, pruning, quantization, finetuning, and fitcompress section `currently maintaimed by TensorFlow only` parameters. By default, the parameters in the config are the following:
