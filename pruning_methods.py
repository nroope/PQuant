...
class ActivationPruning(nn.Module):
    ...
    def __init__(self, config, layer, out_size, *args, **kwargs):
            ...
            self.shape = (1, layer.weight.shape[1]) if self.layer_type == "linear" else layer.weight.shape
            self.mask = torch.ones(self.shape, requires_grad=False).to(layer.weight.device)
            self.activations = None
            self.register_forward_hook(self.collect_output)

    def collect_output(self, module, input, output):
        if not self.training:
            return
        if self.activations is None:
            self.activations = torch.zeros(size=output.shape, dtype=output.dtype, device=self.mask.device)
        self.t += 1
        gt_zero = (output > 0).float()
        self.activations += gt_zero
        if self.t % self.config.t_delta == 0:
            self.t = 0
            pct_active = self.activations / self.config.t_delta
            if self.layer_type == "linear":
                pct_active_avg = torch.mean(pct_active, dim=0)
                self.mask = (pct_active_avg > self.config.threshold).float().unsqueeze(0)
            else:
                pct_active = pct_active.view(pct_active.shape[0], pct_active.shape[1], -1)
                pct_active_avg = torch.mean(pct_active, dim=-1)
                pct_active_above_threshold = (pct_active_avg > self.config.threshold).float()
                self.mask = (pct_active_above_threshold).unsqueeze(-1).unsqueeze(-1)
            self.activations = None
